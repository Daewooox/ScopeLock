...
## Текущий фокус
Задача #0015 — Trialable v0.1 из SA-ревью реализована под контрактом `trialable-v0.1-2026-07-07`: README+LICENSE, детерминированный `scopelock contract new`, наблюдаемость ошибок хука (`hook-errors.ndjson`), компактные Zod-ошибки (`formatZodError` → `INVALID_INPUT`), конфигурируемый `degradedFileThreshold`, npm-подготовка обоих пакетов, кросс-ОС CI (ubuntu/windows/macos × Node 22/24), CLI integration-тесты. core 34/34 + cli 3/3 pass. Интервью Stage 0 сознательно отложены (продукт ещё сырой, нечем «цеплять»). Задача #0018: планы синхронизированы под делегирование. Авторитетный план = `plans/scopelock-implementation-plan.md` → раздел «АКТУАЛЬНЫЙ ПЛАН И СЛЕДУЮЩИЙ ШАГ» (актуальная таблица статусов + пошаговый делегируемый M1-spike с DoD). Продуктовое решение зафиксировано в `strategy-review-round2` (addendum 2026-07-07): интервью Stage 0 отложены, оркестрация (Идея A) = 4-й слой moat и ответ на «почему не Spec Kit/Traycer». Следующая исполнимая задача проекта: M1-spike `globsIntersect` (детали в `plans/orchestration-implementation-plan.md` §2/§5/§8). Ранее: #0017 (PLAN) — implementation-ready план Идеи A. SA-анализ вариантов disjointness, выбран гибрид B+A (NFA product-emptiness + prefix fast-path + консервативный fallback); ядро = `globsIntersect(a,b)`; forbidden в write-write тесте не нужен (`W ⊆ planned`); F1 coloring → F2 послойный; release-gate = консистентность нашего regex с picomatch. Milestones M1-M5, M1-spike первым. Docs-only, контракт `plan-orchestration-impl-2026-07-07`. Дальше по решению пользователя: реализация M1-spike под кодовым контрактом (за checkpoint-gate). Ранее: #0016 creative-формализация Идеи A; #0015 Trialable v0.1; #0014 live UI dogfood. Claude Code strict: forbidden path denied превентивно (файл не тронут), planned path прошёл. Cursor warn: afterFileEdit реально записал audit.ndjson на живую agent-правку. Это закрывает первый пункт checkpoint-gate из #0011. Остаётся перед Phase 4: 5→10-15 интервью Stage 0 (обязательный вопрос "почему не Spec Kit/Traycer") + go/no-go — на пользователе. Housekeeping pending: .pnpm-store/ в .gitignore, решить нужно ли вернуть mode=strict глобально, закоммитить .claude/.cursor настройки от live-теста. Ранее: #0013 Phase 3.5 distribution unblocker (hooks install --local, коммиты 1bd2512/bff915c), #0012 ревью Phase 0-3.
