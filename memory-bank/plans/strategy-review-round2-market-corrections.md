...
## Обновление 2026-07-07 - продуктовое решение и новый moat-трек

> Что изменилось с round 2 по факту реализации. При расхождении верить этому разделу.

### Статус MVP
Детерминированный guardrails-слой из round 2 (§1.2, §3.1) **реализован и подтверждён вживую**:
prompt-контракт → runtime hooks (Claude Code deny, Cursor audit) → post-run drift check.
Trialable v0.1 готов к демонстрации. То есть заявленный moat («deterministic + enforced +
local-first») перестал быть обещанием и стал работающим продуктом.

### Решение 1: интервью Stage 0 отложены (осознанно)
Round 2 ставил интервью как единственный gate перед инвестициями. Пользователь решил
**перенести интервью**: текущего продукта достаточно для демо, но «нечем цеплять» на фоне
Spec Kit/Traycer - нужен явный дифференциатор, которого у них нет. Kill-вопрос
«почему не Spec Kit / Traycer» остаётся обязательным, но зададим его **с более сильным
артефактом на руках**.

### Решение 2: новый дифференциатор - provably parallel-safe оркестрация (Идея A)
Добавляем к moat четвёртый слой поверх трёх из §3.1: **математически доказуемо безопасное
параллельное расписание нескольких агентов** из их glob-scope контрактов (scope-algebra →
conflict graph → раскраска → волны; runtime hook gate как backstop). Это отвечает на реальную
боль «несколько агентов на сложной задаче устраивают бардак», которую **ни Spec Kit, ни
Traycer, ни Kiro не решают детерминированно**. Это и есть ответ на kill-вопрос.
- Теория: `plans/orchestration-scope-algebra.md`.
- План реализации: `plans/orchestration-implementation-plan.md` (M1-M5, начинаем с M1).

### Обновлённая последовательность (product-level)
1. Построить оркестрацию M1-M3 (детерминированное ядро + `plan-parallel` CLI).
2. M4: мини-эксперимент, измерить безопасность/ускорение (falsifiable-гипотезы).
3. **Тогда** интервью Stage 0 - уже с оркестрацией как «крючком» и ответом на «почему не X».
4. go/no-go → Phase 4+ (LLM-планировщик, MCP, templates) по основному техплану.

Elevator-ответ пополняется: «Spec Kit и Traycer планируют для одного агента. Мы даём
доказуемо безопасный параллельный запуск нескольких агентов - никто из них не пишет в один
и тот же файл, и это доказано до запуска, а не выясняется в merge-конфликте.»
