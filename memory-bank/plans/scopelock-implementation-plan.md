...
## АКТУАЛЬНЫЙ ПЛАН И СЛЕДУЮЩИЙ ШАГ (обновлено 2026-07-07) - читать первым

> Это самый свежий раздел. При расхождении с более ранними разделами - верить этому.
> Он написан так, чтобы рутинную работу можно было отдать младшему разработчику или агенту
> без дополнительных вопросов.

### Где мы сейчас (одним взглядом)

- Фазы 0-3 + 3.5 сделаны. Продукт работает end-to-end без LLM: `init → contract new →
  approve → export-prompt/inject-contract → hooks install → check-drift`.
- Live-enforcement подтверждён в реальных Claude Code (strict, блокирует) и Cursor (warn, аудит).
- Trialable v0.1 готов (README/LICENSE, детерминированный `contract new`, наблюдаемость
  ошибок хука, кросс-ОС CI). Тесты: core 34/34 + cli 3/3.
- **Продуктовое решение пользователя (2026-07-07):** интервью Stage 0 ОТЛОЖЕНЫ - продукт пока
  «нечем цеплять». Сначала строим дифференциатор-оркестрацию (Идея A), потом идём в интервью
  с сильным артефактом.

### Что делаем дальше - ровно один трек: Orchestration (Идея A)

Полный, исполнимый план: **`memory-bank/plans/orchestration-implementation-plan.md`**
(теория: `memory-bank/plans/orchestration-scope-algebra.md`).

Порядок milestone'ов там: **M1 → M2 → M3 → M4 → M5**. **Правило: M2+ не начинать, пока
M1 не прошёл soundness-gate.** Ниже - как выполнить **M1** буквально по шагам.

### СЛЕДУЮЩАЯ ЗАДАЧА (делегируемая): M1-spike `globsIntersect`

Цель: честная, консервативная функция «пересекаются ли два glob'а» + property-тесты.
Это единственный рискованный кусок; всё остальное - тривиальная надстройка.

Шаги (выполнять по порядку, не пропускать):

1. **Завести контракт** (дисциплина обязательна - трогаем `packages/**`). Скопировать shape
   из `orchestration-implementation-plan.md` §8, сохранить как staging JSON, затем:
   `node packages/cli/dist/index.js approve <staging>.json`. planned scope: `packages/core/src/schedule/**`,
   `packages/core/src/schedule.test.ts`, `packages/core/src/index.ts`; forbidden: остальное ядро и `packages/cli/**`.
2. **Реализовать** `packages/core/src/schedule/glob-intersect.ts` по алгоритму
   `orchestration-implementation-plan.md` §2.3-2.4:
   - `globsIntersect(a,b)`, `globSetsIntersect(as,bs)`, `intersectionWitness(a,b)`;
   - директорный prefix fast-path (§2.3 шаг 2);
   - general путь: glob→regex (таблица §2.4) → NFA (char-предикаты) → product-BFS emptiness;
   - неподдержанный конструкт → `UnsupportedGlob` → `globsIntersect` возвращает `true` (конфликт).
3. **Экспорт**: добавить одну строку re-export в `packages/core/src/index.ts`.
4. **Тесты** `packages/core/src/schedule.test.ts` по §2.5:
   - unit на известные пары (`*.ts` vs `*.tsx` = disjoint; `src/**` vs `src/api/x.ts` = intersect; и т.д.);
   - property-soundness (если `false` - фаззер путей НЕ находит общий матч; провал = баг);
   - property-consistency: наш regex ≡ `picomatch(g,{dot:true})` на ≥10k случайных пар.
5. **Собрать и прогнать**: `pnpm -r build` → `node --test packages/core/dist/schedule.test.js`.
   Все зелёные? → `node packages/cli/dist/index.js check-drift` (ожидание: 0 нарушений scope).
6. **Зафиксировать**: обновить `tasks.md` (новая задача #0018 M1), `activeContext.md`,
   `component-map.md`; commit + push.

**Definition of Done M1** (gate для перехода к M2):
...
**Если M1 не сходится** (soundness/consistency дорого или недостижимо) - СТОП, не строить M2+;
вернуться к пользователю: вся гарантия Идеи A держится на этом примитиве.
