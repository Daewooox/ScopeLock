...
## Phase 5 - MCP server === СЛЕДУЮЩИЙ ШАГ (делегируемый, competitively-informed)
...
| **Orchestration (Идея A) - новый moat-трек** | **PLAN DONE** (#0016 теория, #0017 impl-план); **следующая работа = M1** | M1 ~1-2 д |
| 4 Manifest + LLM planner | after orchestration + interviews | 3-5 д |
| 5 MCP server | | 2-3 д |
...
## АКТУАЛЬНЫЙ ПЛАН И СЛЕДУЮЩИЙ ШАГ (обновлено 2026-07-09) - читать первым

> Это самый свежий раздел. При расхождении с более ранними разделами - верить этому.
> Написан так, чтобы работу можно было отдать другому агенту без доп. вопросов.

### Где мы сейчас (одним взглядом)

- Фазы 0-3.5 сделаны; продукт работает end-to-end без LLM. Live-enforcement подтверждён
  в реальных Claude Code (strict блокирует) и Cursor (warn аудит).
- **Оркестрация (Идея A) полностью реализована и провалидирована**: M1-M5 done
  (`globsIntersect` → conflict graph → F1/F2 scheduler + cycle detection → `plan-parallel`
  CLI). H1-H5 все GO, включая **H3 на реальных агентах** (#0033): safety под живыми
  агентами (0 коллизий, 4 прогона), speedup ~1.5-1.8x (ниже 3x из-за platform dispatch
  stagger, не планировщика). Плюс `scopelock rebaseline` (#0032), fix baseline-not-found
  (#0031), сквозной parallel-workflow guide (#0030). Тесты: core 53 + cli 14.
- **Продуктовое решение пользователя (актуально 2026-07-09):** Stage 0 интервью пока
  ОТЛОЖЕНЫ - сначала довести до реально полезного продукта. Приоритет следующего шага -
  **MCP-сервер**.

### ⚠️ Конкурентный reality-check (2026-07, ОБЯЗАТЕЛЬНО прочитать)

Веб-скан (`plans/competitive-landscape-2026-07.md`) показал:
- Наш планируемый «MCP scope-enforcement server» — **near-clone `logi-cmd/agent-guardrails`**
  (contracts + scope/test/risk checks + MCP + multi-harness), который feature-complete,
  более интегрирован, и имеет ~8★ (zero traction). Категория scope-enforcement в целом с
  weak PMF.
- Оркестраторы — red ocean (Conductor, Claude Squad, +60 worktree-изоляторов) + платформа
  (Claude Code dynamic workflows) заходит нативно. `scopelock run` строить НЕ стоит.
- Даже наш disjointness-дифференциатор существует (`wit` AST-locking, `swarm-protocol`).
- Multi-agent эффективность академически НЕ доказана (single ≥ multi при равном compute).

Вывод: НЕ строить общий MCP-enforcer вслепую, НЕ строить `scopelock run`, НЕ строить
LLM-планировщик. Дифференциаторы, которые ВЫЖИЛИ и вокруг которых только и строить:
(1) **real-time pre-tool-use deny** (наш hook gate) vs merge-gate конкурентов — ценен в
auto-mode/proactive loops без человека на шаге; (2) **language-agnostic glob-disjointness
+ picomatch-witness** (наш `plan-parallel`) — то, чего нет у enforcer'ов и что у `wit`
только per-language.

### СЛЕДУЮЩАЯ ЗАДАЧА (делегируемая): Phase 5 - MCP server, competitively-informed

Полное ТЗ - в разделе **«Phase 5 - MCP server === СЛЕДУЮЩИЙ ШАГ»** выше (строка ~364).
Порядок жёсткий:

1. **Step 5.0 — buy-vs-build spike (ОБЯЗАТЕЛЬНЫЙ гейт, docs-only, ~1 сессия).** Погонять
   `logi-cmd/agent-guardrails` + `wit` на реальной работе. Записать вердикт в новый
   `plans/mcp-spike-verdict.md`: что ScopeLock даёт, чего они НЕ дают, и что реально
   ощущается нехваткой. **Kill-criterion:** если agent-guardrails + Claude Code native
   hooks/loops покрывают ≥90% — НЕ строить MCP, вернуться к пользователю.
2. **Step 5.1 — MCP server (только если spike = GO), 2-3 дня.** Строго вокруг двух
   дифференциаторов: `plan_parallel`/`scopes_conflict` (уникальный language-agnostic
   scheduler) + `check_drift` как verification-tool для `/goal` + опора на готовый
   real-time hook gate для auto-mode. НЕ дублировать 5 проверок agent-guardrails.
   Позиция — guardrail-слой ДЛЯ платформенных loops, не конкурент оркестраторам.

**DoD** - см. «DoD Step 5» в разделе Phase 5 выше. Всё под ScopeLock-контрактом
(dogfooding), `check-drift` = 0 перед коммитом, push только по явной просьбе.

**Если spike = NO-GO:** не строить MCP. Кандидаты-развилки (обсудить с пользователем):
(a) контрибьютить наш scheduler в существующий инструмент; (b) сузиться до одного
real-time-enforcement-для-auto-mode как единственной фичи; (c) вернуться к отложенному
(Stage 0 с честным «вот что уникального», repo manifest builder).

### После M1

M2 (scope-algebra + conflict graph + F1 coloring + Plan-схема) → M3 (`plan-parallel` CLI) →
M4 (мини-эксперимент из `orchestration-scope-algebra.md` §5, reflection-отчёт, go/no-go) →
M5 (read-hazard рёбра + F2 послойный + детект циклов). Затем - интервью Stage 0 уже с
рабочей оркестрацией как «крючком», и только потом Phase 4+.
