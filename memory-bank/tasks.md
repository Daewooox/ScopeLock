...
<!-- TASK #0037 BEGIN
     ...
     Status: build
-->
## Задача #0037 — Multi-Agent Coordination Benchmark / Flight-Control Experiment

- **Описание:** Создать минимальный воспроизводимый benchmark для ScopeLock Flight Control: fixture repo с 6 задачами (2 independent, 2 write-write conflict, 2 read-write hazard), 3 режима (`without_scopelock`, `contracts_hooks`, `contracts_hooks_plan_parallel`) и метрики scope violations/conflicts/interventions/tests/accepted tasks/wall-clock.
- **Уровень сложности:** Level 2 validation/build.
- **Статус:** BUILD завершён; результаты записаны и закоммичены.
- **Контракт:** `multi-agent-coordination-benchmark-m1` approved at `ac41145`.

### Артефакты
- `benchmarks/coordination/run-benchmark.mjs`
- `memory-bank/plans/multi-agent-coordination-benchmark.md`
...
### Вывод
- Hooks alone remove scope violations but do not solve multi-agent coordination.
- `plan_parallel` converts hidden conflicts into explicit decisions and clean merge-readiness, at the cost of extra coordination/wall-clock in this tiny fixture.
- This supports the Flight Control positioning, but the next proof must use real agents and repeated runs.

### DoD
- [x] Fixture repo generated by benchmark harness.
- [x] 6 tasks represented: 2 independent, 2 write-write conflict, 2 read-write hazard.
- [x] 3 modes executed.
- [x] Metrics table recorded.
...
### Step 2 (production) - CLI `scopelock agents preflight` - DONE

- **Контракт:** `agent-env-preflight-cli-step2` (approve baseline `8edfb0c`, после коммита Step 1). Forbidden `packages/core/**`, `packages/mcp/src/**` - core/MCP не тронуты, тонкая обёртка.
- **Реализовано:**
  - `packages/cli/src/commands/agents-preflight.ts` - читает манифест по пути (та же конвенция, что `plan-parallel`: путь относительно cwd, `MANIFEST_NOT_FOUND`/`INVALID_JSON`/`FILE_READ_ERROR`), валидирует `agentWorkspaceManifestSchema.parse`, опциональный `--target` (repeatable) фильтрует `manifest.targets` с проверкой (`UNKNOWN_TARGET`, если target не в enum или не объявлен в манифесте), вызывает `runAgentPreflight`, валидирует итоговый отчёт через `agentEnvironmentPreflightReportSchema.parse` перед выводом (JSON-контракт проверен схемой, не просто "как есть"). Human-вывод на target: `status`, `rules N/M`, `skills N/M`; под каждым violation - `severity`, `detail` и синтезированный `fix` (конкретная `ruler`/`skills --copy` команда per violation code) - **ничего не выполняется**, только рекомендация. Exit-код: `1` при `violationsCount > 0`, `2` при операционных ошибках (по факту `0` тоже возможен при `status:"warn"` от missing-optional, т.к. warn не создаёт violation-записи).
  - `packages/cli/src/index.ts` - команда `agents preflight --manifest <path> [--target <id>] [--json]`.
  - `README.md` - строка в таблице команд + абзац про read-only/fix-hint семантику.
- **Тесты:** +8 в `packages/cli/src/cli.test.ts` (`describe("agents preflight")`): all-pass 3 targets; missing required skill → exit 1 + `missing_required_skill` violation; missing optional → `warn`, 0 violations, exit 0; `--target` фильтрует до 1 таргета; unknown `--target` → exit 2 `UNKNOWN_TARGET`; отсутствующий manifest → exit 2 `MANIFEST_NOT_FOUND`; невалидная схема (плохой target enum) → exit 2 `INVALID_INPUT` (через `formatZodError`, как у остальных команд).
- **Проверки:** `pnpm -r build` + `pnpm typecheck` чисты; core 70/70 (не менялся), cli **26/26 (+8)**, mcp 3/3; `git diff --check` чист; ручной smoke (temp git repo): все-pass, missing-required, `--target`-фильтр, unknown-target, missing-manifest - вывод и exit-коды совпали с ожиданиями до автотестов.
- **Ограничения / отложено:** нет harness discovery/version probe; `hookConfidence` не заполняется (Step 3); никаких мутаций/materializer-вызовов - только `fix`-подсказки текстом.

### Step 3a (production) - hook capability model + config probe - DONE (Codex hook file adapter СОЗНАТЕЛЬНО ОТЛОЖЕН)

- **Контракт:** `agent-env-hook-capabilities-step3` (approve baseline `dcf6024`, после Step 2). Forbidden `git/**`, `drift/**`, `hook/**`, `schedule/**`, `harness/{claude-hooks,cursor-hooks,hooks-merge}.ts`, `packages/mcp/src/**` - install/uninstall-механизм и MCP не тронуты.
- **Ключевое решение (сузил скоуп после доп. проверки доков):** официальные доки Codex (developers.openai.com/codex/config-advanced#hooks, зафетчено 2026-07-10) подтвердили TOML `[[hooks.PreToolUse]]` формат, но явно сказали, что **JSON-схема `.codex/hooks.json` НЕ задокументирована**, событие PreToolUse для file-edit тула `apply_patch` **не задокументировано и не поймано вживую** (Step 0 пробовал только `Bash`-tool), а механизм project-trust **нигде не хранится читаемо на диске**. Строить hook-адаптер (install/uninstall/парсинг события) на угадывании этих трёх вещей - риск тихо-неработающего enforcement (хуже, чем отсутствие enforcement). Решение: реализовать **только** то, что подтверждено доками+Step-0-evidence (capability-модель + статичный config-probe), **сознательно отложить** сам Codex hook file adapter до отдельного live-суб-спайка, который поймает реальное `apply_patch` PreToolUse событие.
- **Реализовано:**
  - `packages/core/src/harness/capabilities.ts` (новый) - `NOMINAL_HOOK_CAPABILITIES: Record<AgentId, HookCapabilities>`, чистый/статичный, `confidence` всегда `"documented"` в номинальной таблице (никогда `"live-verified"` - это отдельный явный ручной шаг). Cursor `canDeny: false` навсегда (Step 0 не смог его живо проверить - план явно запрещает апгрейдить без доказательства).
  - `packages/core/src/agents/hook-probe.ts` (новый) - `probeHookConfig(repoRoot, target)`: только чтение конфиг-файлов (`.claude/settings.json` через существующий `hasScopeLockHooks`/`hooksConfigPath`; для codex - просто наличие `.codex/hooks.json`/`config.toml`), без process exec/сети. Codex **всегда** `confidence: "degraded"`, `installed: false` (мы ничего не пишем и не можем подтвердить чужой конфиг) - независимо от того, есть ли файл конфига.
  - `packages/core/src/schemas/agent-workspace.ts` - `hookCapabilitiesSchema`, `hookConfigProbeSchema` (Zod); `targetPreflightReportSchema` получил обязательное поле `hook: hookConfigProbeSchema` (аддитивное расширение Step 1/2 схемы, `schemaVersion` не поднимал - пре-релизный формат, старые тесты не ломались, т.к. использовали point-assertions, не deepEqual).
  - `packages/core/src/agents/preflight.ts` - `runAgentPreflight` теперь вызывает `probeHookConfig` на каждый target и кладёт в `TargetPreflightReport.hook`; hook-confidence НЕ влияет на `status`/`rollUp` в этом шаге (informational, gating - Step 4 по плану).
  - `packages/core/src/harness/registry.ts` - исправлен протухший `codex: hooksSupport: "none"` → `"deny"` (поле мёртвое, консьюмеров нет, но плана явно называл его stale) с комментарием-ссылкой на новую модель.
  - `packages/cli/src/commands/agents-preflight.ts` - human-вывод получил строку `hook: <mode> (<confidence>[, installed])` на каждый target (`pre-write deny` / `post-write audit only` / `no enforcement`); JSON автоматически несёт `target.hook` через существующую Zod-валидацию отчёта.
  - `README.md` - абзац про `hook`-поле, честно объясняющий, почему codex всегда `degraded`.
- **Тесты:** +6 core (`packages/core/src/agent-preflight.test.ts`, `describe("hook capability model (Step 3)")`: номинальная таблица никогда не `live-verified`; cursor никогда `canDeny`; claude/cursor `documented` без установленного хука; claude-проба реально видит установленный хук (`installHooks` + `probeHookConfig`); codex всегда `degraded` вне зависимости от наличия `config.toml`; `runAgentPreflight` кладёт `hook` per-target без влияния на `status`). +1 CLI (`packages/cli/src/cli.test.ts`: реальный `hooks install --target claude --local` → preflight видит `installed: true, confidence: documented`) + расширена существующая all-pass проверка assertions на `hook.capabilities` по всем трём targets.
